...
function MenuItem({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className="press-scale flex w-full items-center gap-3 rounded-[var(--radius-md)] px-3 py-2.5 text-left text-body text-ink active:bg-cream-dark/60 disabled:opacity-40 dark:text-ink-dark dark:active:bg-surface"
    >
...
export function ListActionMenu({
  ...
  return createPortal(
    <>
      <div
        className="viewport-overlay z-50 bg-[var(--color-overlay)]"
        onClick={onClose}
        aria-hidden="true"
      />
      <motion.div
        ...
        className="fixed z-50 min-w-[14rem] w-56 overflow-hidden rounded-[var(--radius-lg)] border border-separator bg-cream py-1.5 shadow-lg dark:bg-surface-raised"
        style={{
